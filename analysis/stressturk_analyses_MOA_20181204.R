...
# gdata = read.csv("stressTurk_data_20181204.csv", header  = T, na.strings="--undefined--")
...
process_data = function(file_name){
  ...
  relevant_columns = c('participant','item','condition','duration','meanIntensity','meanpit', 'index', 'list')
...
run_regression = function(df,observation){
  r = lmer(get(observation) ~ condition + (1 + condition | participant) + (1 + condition | item) + (1 + condition | list), data=df)
  summary(r)
}
...
file_name = 'stressTurk_data_20181204_sort.csv'
...
plot_data(summarized_dataset_duration,title='Effect of contrast on duration: All Trials of Each Condition')