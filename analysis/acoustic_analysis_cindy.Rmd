...
info = c('participant','item_id','position_condition', 'word', 'word_num')
bRemove_outliers = 0
library(dplyr)
library(lme4)
library(zeallot)
...
df0$position_condition = NA
df0$item_id = NA

for (iR in 1:nrow(df0)){
  df0$position_condition[iR] =  as.character(tAll_trials[tAll_trials$trial_id == df0$trialId[iR],'position_condition'])
...
process_data = function(df){
  ...
  # df_Agent = df[(df$position_condition=='Agent' | df$position_condition=='Verb') & df$word_num=='3',]
  # 
  # df_Verb = df[(df$position_condition=='Verb'| df$position_condition=='Patient') & df$word_num=='5',]
  # 
  # df_Patient = df[(df$position_condition=='Patient'| df$position_condition=='Agent') & df$word_num=='6',]
  
  df_Agent = df[(df$position_condition=='Agent' | df$position_condition=='Control') & df$word_num=='3',]
  # df_Agent inheri row hum from df
  
  df_Verb = df[(df$position_condition=='Verb'| df$position_condition=='Control') & df$word_num=='5',]
  
  df_Patient = df[(df$position_condition=='Patient'| df$position_condition=='Control') & df$word_num=='6',]
...
c(df_Verb, df_Agent, df_Patient) %<-% process_data(df2)



combine_datasets = function(Agent,Verb,Patient){
  Agent$condition = mapvalues(Agent$position_condition,c('Agent'),c('contrast'))
  Verb$condition = mapvalues(Verb$position_condition,c('Verb'),c('contrast'))
  Patient$condition = mapvalues(Patient$position_condition,c('Patient'),c('contrast'))
...
plot_data = function(d,feature, title){
  print(ggplot(d, aes(x=Location, y=get(feature), fill=condition)) + 
    ...
    scale_fill_hue(name="position_condition", 
...
cat("  \n###", observation, "of", location, "  \n")
    # r = lmer(get(observation) ~ position_condition + (1 + position_condition|participant) + (1 + position_condition | item_id), data=get(paste0("df_", location)))
        r = lmer(get(observation) ~ position_condition  + (1 + position_condition | item_id), data=get(paste0("df_", location)))
  print(summary(r))
  summary(r)
  cat("  \n")
}
