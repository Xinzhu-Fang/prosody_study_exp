...
gdata = read.csv("measure20181204.csv", header  = T)
...
process_data = function(file_name){
  ...
  df_agent = df[(df$condition=='agent' | df$condition=='baseline') & df$word_num=='2',]
  
  df_verb = df[(df$condition=='verb'| df$condition=='baseline') & df$word_num=='4',]
  
  df_patient = df[(df$condition=='patient'| df$condition=='baseline') & df$word_num=='5',]
  
  
  relevant_columns = c('participant','item','condition','duration','meanIntensity','meanpit')
...
file_name = 'stressTurk_data_20181204.csv'
...
plot_data(summarized_dataset_duration,title='Effect of contrast on duration: Second Instances of Each Condition')
